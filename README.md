...
- Die stärksten offenen Risiken liegen derzeit bei `CMS/install.php`; der frühere Direkt-Fetch des eingebauten Importers wurde inzwischen auf die zentrale Core-HTTP-Härtung umgestellt, die Audit-Dokumentation referenziert den verifizierten Scope konsistent über `FILEINVENTAR.md`, Debug-Logs liegen außerhalb des Release-Baums, der GitHub-Doku-Sync akzeptiert nur noch freigegebene `/DOC`-Bundles am festen Repo-Root-Zielpfad, und der SSRF-Schutz blockiert ungelöste Remote-Hosts jetzt standardmäßig auf Release `2.6.8`.
