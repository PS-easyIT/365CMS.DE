...
| **Pages & Posts** | Content structure, editorial content, comments and supporting content blocks | Create, edit, organize, publish content, moderate comments with quick filters and keep slug/taxonomy links stable; the desktop editor now starts top-aligned in both pages and posts with a compact right sidebar and no large vertical gaps. In `SEO Score & Checklist`, the four KPI tiles above `Live hints` are rendered in a single vertical column, and every SEO/sidebar panel after `Publication` starts collapsed by default (while the publication card itself remains normally visible). The SEO default notice remains intentionally subtle with a clear title above the actions; reset buttons stay side-by-side on desktop and wrap safely for longer labels without horizontal overflow |
