...
![Security Score](https://img.shields.io/badge/Security-96.20-15803d)
![Speed Score](https://img.shields.io/badge/Speed-93.26-2563eb)
![PHP Score](https://img.shields.io/badge/PHP%20%2F%20Best%20Practices-97.52-7c3aed)
![Overall Score](https://img.shields.io/badge/Overall-97.61-0f766e)
...
365CMS verbindet klassisches CMS-Content-Management, Mitgliederbereiche, SEO-/Performance-Werkzeuge, Theme-Customizing, Plugin-Erweiterbarkeit und Betriebswerkzeuge in einer konsistenten PHP-Architektur. Der Fokus liegt auf **betriebssicherem Self-Hosting**, **klarer Admin-Struktur**, **lokal dokumentierten Workflows** und **sauber integrierbaren Erweiterungen**.
...
| Bereich | Highlights |
|---|---|
| **Core** | Modulare PHP-Architektur, Service-Layer, Routing, Hook-System, Konfiguration, Logging und zentrale Systembausteine |
| **Admin** | Zentrales Dashboard sowie Verwaltungsbereiche für Inhalte, Benutzer, Medien, SEO, Performance, Recht, System, Diagnose und Updates |
| **Content** | Seiten, Beiträge, Landing-Pages, Kategorien, Tags, Revisionen, Featured Images, TOC und mehrsprachige Inhalte mit Editor.js-Unterstützung |
| **Medien** | Medienbibliothek mit Uploads, Ordnern, Kategorien, Bild-/Dateiverwaltung, Grid-/Listenansicht und WebP-Workflows |
| **Mitgliederbereich** | Member-Dashboard, Profile, Widgets, Benachrichtigungen, Onboarding, Profilfelder und Frontend-Module |
| **Suche** | Volltextsuche mit TNTSearch, Indexierung zentraler Inhalte und mehrsprachigen Suchfeldern |
| **Security & Auth** | Rollen und Rechte, CSRF-/XSS-Schutz, Rate Limits, Firewall, AntiSpam, Passkeys, 2FA, LDAP und sichere Login-/Reset-Flows |
| **SEO** | Meta-Daten, Social-Media-Daten, strukturierte Daten, Sitemap, Robots, Redirects, 404-Monitor, Analytics und IndexNow |
| **Business** | Aboverwaltung, Pakete, Bestellungen, Checkout, Gruppen, Mitgliederfunktionen und geschäftsnahe CMS-Prozesse |
| **Legal & DSGVO** | Legal-Sites, Cookie-Manager, Consent-Dialog, Datenschutzprozesse, Datenexporte und Löschanfragen |
| **Themes & Design** | Theme-System, Customizer, Theme-Explorer, Code-Editor, Menü-Editor, Landing-Page-Editor, Font-Manager, CMS-Loginpage und Theme-Marketplace |
| **Plugins** | Erweiterbares Plugin-System mit Hooks, Plugin-Verwaltung, Marketplace und modularen Integrationen |
| **Betrieb & Wartung** | Cache- und Performance-Werkzeuge, Cron, Monitoring, Health-Checks, Backups, Mail-Queue, Logs und Update-Management |

### 🏗️ Architektur & Workspace

Die aktuelle Arbeitsumgebung ist auf drei Repositories verteilt:
...
| `CMS/member/` | Mitgliederbereich, Controller und Views |
...
> Hinweis: Die produktiv gebündelten Symfony-Komponenten unter `CMS/assets/` (`mailer`, `mime`, `translation`) verlangen PHP 8.4. 365CMS prüft diese Mindestplattform bereits vor dem regulären Bootstrap.
...
| Bereich | Stand |
|---|---|
| **Security** | CSRF, XSS-Escaping, Prepared Statements, Login-Limits, Firewall, Audit-Log |
...
| **Auth** | Rollen, Capabilities, Member-Dashboard, LDAP, TOTP, Passkeys/WebAuthn |
...
Zusätzlich bauen SERP-, Sitemap- und IndexNow-Hinweise ihre öffentlichen Beispiel- und Status-URLs inzwischen runtime-aware auf: Proxy-, Alternativhost- und lokale Dev-Umgebungen sehen damit dieselbe aktuelle Public-Origin wie der laufende Request, statt auf an `SITE_URL` festgeklebte SEO-Hilfsziele zu kippen.
...
365CMS combines classic CMS content management, member areas, SEO and performance tooling, theme customization, plugin extensibility and operational tooling in one consistent PHP architecture. The focus is on **reliable self-hosting**, **clear admin workflows**, **locally maintained documentation** and **clean extension points**.
...
| Area | Highlights |
|---|---|
| **Core** | Modular PHP architecture, service layer, routing, hook system, configuration, logging and central system building blocks |
| **Admin** | Central dashboard plus management areas for content, users, media, SEO, performance, legal, system, diagnostics and updates |
| **Content** | Pages, posts, landing pages, categories, tags, revisions, featured images, TOC and multilingual content with Editor.js support |
| **Media** | Media library with uploads, folders, categories, image/file management, grid/list views and WebP workflows |
| **Member Area** | Member dashboard, profiles, widgets, notifications, onboarding, profile fields and frontend modules |
| **Search** | Full-text search with TNTSearch, indexing of core content and multilingual search fields |
| **Security & Auth** | Roles and permissions, CSRF/XSS protection, rate limits, firewall, anti-spam, passkeys, 2FA, LDAP and secure login/reset flows |
| **SEO** | Meta data, social media data, structured data, sitemap, robots, redirects, 404 monitor, analytics and IndexNow |
| **Business** | Subscription management, packages, orders, checkout, groups, member features and business-oriented CMS workflows |
| **Legal & GDPR** | Legal pages, cookie manager, consent dialog, privacy workflows, data exports and deletion requests |
| **Themes & Design** | Theme system, customizer, theme explorer, code editor, menu editor, landing page editor, font manager, CMS login page and theme marketplace |
| **Plugins** | Extensible plugin system with hooks, plugin management, marketplace and modular integrations |
| **Operations & Maintenance** | Cache and performance tools, cron, monitoring, health checks, backups, mail queue, logs and update management |

### 🏗️ Architecture & Workspace

The current workspace is split into three repositories:
...
| Path | Content |
|---|---|
| `CMS/core/` | Core classes, router, security, database, services |
| `CMS/admin/` | Admin pages, modules, partials, diagnostics, UI |
| `CMS/member/` | Member area, controllers and views |
| `CMS/assets/` | Production vendor bundles and runtime assets |
| `DOC/` | Admin, core, theme, member, workflow and audit documentation |
...
> Note: The bundled Symfony runtime components inside `CMS/assets/` (`mailer`, `mime`, `translation`) require PHP 8.4. 365CMS validates this platform baseline before the normal bootstrap continues.

#### Installation

1. Copy the project to your web server.
2. Provide database credentials in `config.php` or through the installer.
...
- Verify HTTPS and rewrite configuration
- Make cache, upload and log directories writable
- Configure mail transport
- Optionally enable LDAP in `CMS/config/app.php`
- Select a theme and review the theme customizer

### 📚 Documentation

Start with the local documentation index at [`DOC/INDEX.md`](DOC/INDEX.md).

| Topic | Document |
|---|---|
| Project overview | [`DOC/README.md`](DOC/README.md) |
| Installation | [`DOC/INSTALLATION.md`](DOC/INSTALLATION.md) |
| Core architecture | [`DOC/core/ARCHITECTURE.md`](DOC/core/ARCHITECTURE.md) |
| Services | [`DOC/core/SERVICES.md`](DOC/core/SERVICES.md) |
| Hooks | [`DOC/core/HOOKS-REFERENCE.md`](DOC/core/HOOKS-REFERENCE.md) |
| Admin area | [`DOC/admin/README.md`](DOC/admin/README.md) |
...
### 🛡️ Security, Operations & Maintenance

| Area | Status |
|---|---|
| **Security** | CSRF, XSS escaping, prepared statements, login limits, firewall, audit logging |
| **Operations** | Cache management, DB checks, cron monitoring, disk usage, health checks |
| **Mail** | Symfony Mailer/Mime, queue handling, retry backoff, transport diagnostics |
| **Auth** | Roles, capabilities, member dashboard, LDAP, TOTP, passkeys/WebAuthn |
| **Compliance** | Cookie consent, data export, deletion requests, legal pages, `security.txt` |

### 🧪 Technology Stack & Important Bundles

| Area | Components |
|---|---|
| **Backend** | PHP 8.4+, PDO, MySQL/MariaDB |
| **Mail** | Symfony Mailer, Symfony Mime, mail queue, Microsoft 365 / XOAuth2 preparation |
| **Security/Auth** | HTMLPurifier, JWT, WebAuthn, TwoFactorAuth, LdapRecord |
| **Editor & UI** | SunEditor, Editor.js, Tabler, Grid.js |
| **Media** | Internal upload/picker APIs, PhotoSwipe, WebP workflows |
...
SERP previews, sitemap hints and IndexNow status links are also resolved against the current public runtime base now, so admin diagnostics stay on the active public origin even behind proxies, alternate hosts or local development URLs.
