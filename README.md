...
[![Version](https://img.shields.io/badge/version-2.6.12-blue.svg)](Changelog.md)
...
- Die stärksten offenen Risiken liegen derzeit nicht mehr in einem monolithischen `CMS/install.php`, sondern in den noch offenen Restpunkten ab A-13; der Installer läuft jetzt über Bootstrap + Controller/Service/Views, der frühere Direkt-Fetch des eingebauten Importers wurde bereits auf die zentrale Core-HTTP-Härtung umgestellt, die Audit-Dokumentation referenziert den verifizierten Scope konsistent über `FILEINVENTAR.md`, Debug-Logs liegen außerhalb des Release-Baums, der GitHub-Doku-Sync akzeptiert nur noch freigegebene `/DOC`-Bundles am festen Repo-Root-Zielpfad, der SSRF-Schutz blockiert ungelöste Remote-Hosts standardmäßig, `session.cookie_secure` folgt echtem HTTPS, Core-Updates laufen staging-basiert mit atomarem Ziel-Swap bzw. Rollback und die HTTPS-/HSTS-Linie ist jetzt verbindlich auf Upstream-/Proxy-Redirect plus HTTPS-erkanntes HSTS im Core ausgerichtet — auf Release `2.6.12`.
