...
### User-friendly
Hooks let plugins and themes extend 365CMS without changing core files. Actions notify listeners about lifecycle events; filters let integrations adjust values before they are used.

### Technical
The dispatcher is implemented by `CMS/core/Hooks.php`; Bootstrap and routing provide lifecycle extension points, while plugin and dashboard registries expose integration contracts. The complete detailed reference is retained in the German technical section below.

## Deutsch

### Anwenderfreundlich
Hooks erlauben Plugins und Themes, den Core ohne direkte Änderungen zu erweitern. Actions melden Ereignisse; Filter verändern Werte vor ihrer Verwendung.
...
Die Referenz beschreibt die aktuelle 3.4.00-Hook-Landschaft auf konzeptioneller Ebene. Für Debugging und harte Laufzeitverträge sind die tatsächlichen `doAction()`-/`applyFilters()`-Aufrufe im Core und in den Admin-/Service-Modulen maßgeblich.

## Verifizierte Bootstrap-Hooks `3.4.00`

Der Bootstrap löst verbindlich `cms_init`, `cms_init_{mode}`,
`cms_before_route`, `register_routes` und `cms_after_route` aus. Zusätzlich
registriert der Core die Cron-Hooks `cms_cron_mail_queue`,
`cms_cron_hourly` und `cms_cron_daily`. Diese Hooks sind gegenüber
konzeptionellen Beispielen die führende Runtime-Referenz.
