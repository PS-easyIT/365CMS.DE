...
### User-friendly
365CMS protects requests with hardened sessions, CSRF tokens, capability checks, output escaping, upload validation, rate limits and audit trails. Keep secrets outside the repository and review logs after updates.

### Technical
The security boundary spans `Security`, `Auth`, `AuditLogger`, `Http\Client`, upload/media services and router middleware. Web Vitals enforce same-origin, payload-size and request-rate limits; administrative mutations require capability and CSRF checks. The complete detailed reference is retained in the German technical section below.

## Deutsch

### Anwenderfreundlich
365CMS schützt Anfragen durch gehärtete Sessions, CSRF-Token, Rechteprüfungen, Escaping, Upload-Validierung, Rate-Limits und Audit-Spuren. Geheimnisse gehören außerhalb des Repositories.
...
Die Datei beschreibt die aktuell beabsichtigte Sicherheitsarchitektur der 3.4.00-Laufzeit. Für Incident-Analyse und harte Wahrheit gelten immer zusätzlich die produktiven Entry-Points, Router-Prüfungen, Services und Admin-Handler der laufenden Installation.
...
| PHP 8.4 `strict_types` | In allen Core-Dateien |
...
Die Security-Zweitprüfung für `3.4.00` hat folgende Punkte gehärtet:
...
### Verifizierte Runtime-Sicherheitsflächen `3.4.00`

Die aktuelle Prüfung umfasst neben Session-, CSRF-, Header-, Upload- und
Prepared-Statement-Schutz auch:

- Same-Origin-Prüfung und Payload-Limit für Web-Vitals
- CSRF-Prüfung für administrative Mail- und Graph-Test-Endpunkte
- capability-basierte Admin-API-Prüfungen
- zentrale Antispam-, Firewall- und Security-Alert-Services
- Audit-Logging über `AuditLogger` und getrennte Security-Runtime-Regeln
