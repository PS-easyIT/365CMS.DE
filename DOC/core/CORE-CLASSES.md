...
### User-friendly
This reference explains the core building blocks that make 365CMS run: bootstrap, routing, database access, authentication, security, hooks, plugins, themes and shared services.

### Technical
The current inventory includes root classes plus `Auth/`, `Contracts/`, `Http/`, `Member/`, `Routing/` and the complete `Services/` tree, including AI, SEO, media and landing modules. The complete detailed reference is retained in the German technical section below.

## Deutsch

### Anwenderfreundlich
Diese Referenz erklärt die Bausteine des 365CMS-Core: Startlogik, Routing, Datenbank, Anmeldung, Sicherheit, Hooks, Plugins, Themes und Services.
...
### Inventurhinweis `3.4.00`

Die Klassenreferenz wurde gegen den vollständigen Bestand unter `CMS/core/`
geprüft. Der Core umfasst 155 PHP-Dateien; davon liegen 112 im
Service-Bereich. Neue oder spezialisierte Klassen sind in den jeweiligen
Service-/Routing-Abschnitten nach Verantwortungsbereich gruppiert, damit die
Referenz nicht nur die zentralen Singleton-Fassaden, sondern auch die
Runtime-Erweiterungen abbildet.

```php
CMS\Version::CURRENT;       // '3.4.00'
CMS\Version::RELEASE_DATE;  // '2026-09-05'
...
*Generiert am 2026-09-06 – Diese Dokumentation basiert auf dem aktuellen Stand des
365CMS-Quellcodes (Version 3.4.00).*
