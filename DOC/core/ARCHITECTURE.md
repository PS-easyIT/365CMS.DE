...
### User-friendly
365CMS starts through one bootstrap, selects a request mode, loads only the required subsystems and dispatches the request through the core router. Plugins, themes and services extend the system through documented hooks and container bindings.

### Technical
The verified lifecycle is `cms_init` → `cms_before_route` → `register_routes` → dispatch → `cms_after_route`. `Bootstrap` detects `web`, `admin`, `api` or `cli`, validates bundled PHP platform requirements, initializes the container and conditionally loads themes. The complete detailed reference is retained in the German technical section below.

## Deutsch

### Anwenderfreundlich
365CMS startet über einen zentralen Bootstrap, erkennt den Betriebsmodus und leitet Anfragen über den Core-Router weiter. Plugins, Themes und Services werden über Hooks und den Container erweiterbar eingebunden.
...
### Verifizierter Bootstrap-Vertrag `3.4.00`

`Bootstrap` initialisiert den Modus, prüft Plattform- und Bundle-Anforderungen,
registriert die Core-Hooks, lässt Plugins über `register_routes` Routen ergänzen
und führt anschließend den Router aus. Nach dem Dispatch wird
`cms_after_route` ausgelöst. Die maßgebliche Reihenfolge lautet:
`cms_init` → `cms_before_route` → `register_routes` → Dispatch →
`cms_after_route`.
...
## Systemschichten <!-- UPDATED: 2026-09-06 -->
...
## Request-Lifecycle <!-- UPDATED: 2026-09-06 -->
...
## Dependency Injection / Service Container <!-- UPDATED: 2026-09-06 -->
...
## Autoloading (PSR-4, Composer) <!-- UPDATED: 2026-09-06 -->
...
Für den produktiven Betrieb sind außerdem die Runtime-Pfade maßgeblich: aktive Themes werden aus `CMS/themes/<slug>/`, aktive Plugins aus `CMS/plugins/<slug>/` geladen. Separate Quell-Repositories wie `365CMS.DE-THEME/` oder `365CMS.DE-PLUGINS/` sind Entwicklungs- bzw. Quellbestände und nicht automatisch Teil der Live-Laufzeit.
...
## Konfigurationsmanagement <!-- UPDATED: 2026-09-06 -->
...
| `CMS_VERSION` | Aktuelle Version | `3.4.00` |
...
## Fehlerbehandlung und Logging <!-- UPDATED: 2026-09-06 -->
