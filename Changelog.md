...
| **2.8.1** | 🔴 fix | Core/Cron | **`CMS/cron.php` feuert den bestehenden Hook `cms_cron_mail_queue` jetzt bei jedem Mail-Queue-/`all`-Lauf wirklich ab, ohne die Mail-Queue doppelt zu verarbeiten**: Core und Plugins können damit an häufigen Cron-Ticks andocken, während `MailQueueService` über das neue Kontext-Flag erkennt, dass der eigentliche Mail-Worker im aktuellen Lauf bereits direkt ausgeführt wurde. |
| **2.8.1** | 🔴 fix | Core/Cron | **Der Core überbrückt ältere `cms-feed`-Deployments jetzt rückwärtskompatibel direkt**: Wenn das bereits deployte Feed-Plugin den neuen `cms_cron_mail_queue`-Drain-Worker noch nicht registriert, ruft `CMS/cron.php` die bestehende `CMS_Feed_Cron::process_queue()`-Routine bei häufigen Mail-Queue-/`all`-Ticks einmal pro Lauf direkt auf und verhindert so, dass die Feed-Queue nur stündlich in 5er-Batches schrumpft. |
| **2.8.1** | 🔴 fix | Core/Cron | **Der Core räumt hängen gebliebene `cms-feed`-Queue-Jobs jetzt auch ohne Plugin-Update wieder frei**: `CMS/cron.php` setzt verwaiste `feed_fetch_queue`-Einträge, die länger als 20 Minuten auf `processing` stehen, vor dem rückwärtskompatiblen Feed-Bridge-Lauf wieder auf `pending`, damit überfällige Feeds nach abgebrochenen Cron-Läufen oder FTP-only-Core-Deployments nicht dauerhaft blockiert bleiben. |
| **2.8.1** | 🔴 fix | Core/System | **Der Systembereich kann `cron.php` jetzt selbst über mehrere Mechanismen ausführen**: Auf `Monitoring → Cron-Status` stehen nun direkte Core-Läufe ohne HTTP, echte HTTP-Loopback-Tests gegen `/cron.php` sowie ein Ajax-Runner ohne Seitenreload bereit, damit Cron-Debugging im CMS auch ohne externen Host-Cron oder Shell-Zugang möglich bleibt. |
| **2.8.1** | 🔵 docs | Audit/Live | **`DOC/audit/PHINIT-LIVE-AUDIT-2026-04-03.md` dokumentiert einen Produktions-Snapshot der live deployten Site `https://phinit.de/`**: festgehalten wurden u. a. gemischte DE-/EN-Archive, unübersetzte EN-Pflichtseiten, fehlerhafte Sprachwechsel und die noch zu breite Live-Suche für präzise Fachbegriffe. |
| **2.8.1** | 🔵 docs | Audit/Live | **`DOC/audit/PHINIT-LIVE-LOCALIZATION-2026-04-03.md` und `DOC/audit/PHINIT-LIVE-SEARCH-ARCHIVE-2026-04-03.md` zerlegen den Live-Befund in Localization-/Routing- sowie Search-/Archive-Teilberichte**: konkrete URLs, 404-Ziele, Duplicate-Content-Risiken und UI-Sprachmischungen sind damit reproduzierbar abgelegt. |
| **2.8.1** | 🔵 docs | README/Audit | **`DOC/audit/BEWERTUNG.md` und `README.md` spiegeln den separaten Live-Snapshot von `phinit.de` jetzt zusätzlich**: Die bestehende Core-/Repo-Bewertung bleibt erhalten, wird aber um einen klar abgegrenzten Produktions-Score samt Verlinkung auf die neuen Auditberichte ergänzt. |
