...
| **3.3.76** | 🔴 fix | EditorJS / Trennstrich | **Der Trennstrich blieb unabhängig von der gewählten Einstellung als mittiger 25%-Strich sichtbar.** `CmsDelimiterTool` normalisiert jetzt `style`, `lineWidth` und `lineThickness` beim Speichern und löst nach jeder Auswahl explizit EditorJS-/CMS-Change-Events aus, damit die Werte zuverlässig im Hidden-JSON landen. Der Renderer ergänzt sanitizer-feste Klassen wie `editorjs-delimiter--width-60` und `editorjs-delimiter--thickness-3`; `editorjs-content.css` rekonstruiert daraus Breite/Dicke auch dann, wenn ein Theme/Purifier Inline-CSS entfernt. Der vendorte 25%-Default wurde zusätzlich auf den systemweit verwendeten 35%-Standard vereinheitlicht. |
