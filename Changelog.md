...
| **3.4.00** | 🛡️ security | Abschlussprüfung / AI / Updates | Abschließende Härtung der 3.4.00-Änderungen: automatischer Updater arbeitet mit installationsweiter Exklusivsperre, zwingender SHA-256-Prüfung, DNS-Pinning gegen Rebinding, Cache-Invalidierung und erhaltener Recovery-Kopie bei partiellem Rollback. Schema-Marker können weder über den Admin-Updater noch den regulären Migrationspfad zurückgesetzt werden und werden erst nach Prüfung von `ai_quota_usage` bestätigt. AI zählt Provider-Aufrufe tatsächlich pro Batch/Retry/Fallback, behält HTTP-429/5xx für die Wiederholungsklassifikation, erzwingt Locale-Gates für SEO/Content und zeigt nur policy-/secret-/endpoint-bereite Funktionen an. Legacy-Provider-Secrets werden bei Löschung vollständig entfernt. |
| **3.4.00** | 🟢 feat | AI / Provider-Betriebsprofile | Provider und Modelle können jetzt das Betriebsprofil **„Alle Funktionen“** wählen. Es ist zentral validiert, persistent und im Admin-Dropdown verfügbar; die Freigabe umfasst alle bereits aktivierten Provider-Scopes, ohne globale Features, Rollenrechte, Locale-Allowlists, Quotas sowie Beta- oder External-Egress-Gates zu umgehen. |
| **3.4.00** | 🔴 fix | AI / Editor.js-Übersetzung | Live-Modelle verarbeiten Editor.js-Übersetzungen jetzt in konservativen Batches mit höchstens 2.400 Zeichen bzw. 12 Segmenten. Der Parser akzeptiert zusätzlich sichere indexierte Antwortobjekte und ordnet sie vor der Übernahme deterministisch zurück; unvollständige oder nicht lesbare Batch-Antworten lösen zentral Retry und Fallback aus, statt den ersten Lauf endgültig abzubrechen. |
| **3.4.00** | 🔴 fix | Runtime / Kompatibilität | Produktivwarnungen wurden geschlossen: sichere Regex-Delimiter für Landing-/Theme-Pfade, MariaDB-kompatible Tabellenchecks, fehlertolerante Quote-Defaults im EditorJS-Normalizer/Sanitizer sowie fail-closed Scalar-Validierung für Post-/Page-Eingaben und CSRF-Tokens. Ein neuer Runtime-Regressionstest verhindert Rückfälle. |
| **3.4.00** | 🔴 fix | Plugins / EditorJS | Wiederholte Includes über abweichende Deployment- oder Case-Pfade führen für die betroffenen Plugin-Klassen nicht länger zu `Cannot redeclare`-Fatals; die Klassendateien kehren nach einer autoload-freien Guard-Prüfung kontrolliert zurück. Der Beitragseditor ignoriert außerdem nicht skalare EditorJS-Blockwerte beim Auszug, statt eine `Array to string conversion`-Warnung zu erzeugen. |
