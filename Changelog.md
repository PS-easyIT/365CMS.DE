...
| **2.8.0 RC** | 🟢 feat | SEO/IndexNow | **`CMS/admin/modules/seo/SeoSuiteModule.php`, `CMS/core/Services/IndexingService.php`, `CMS/admin/views/seo/technical.php` und `CMS/admin/views/seo/sitemap.php` ergänzen eine pflegbare IndexNow-Konfiguration mit API-Key, Root-`.txt`-Auswahl und Validierung im SEO-Bereich**: Der Admin kann den Schlüssel jetzt direkt speichern, vorhandene Root-Keydateien im CMS-Webroot auswählen und prüfen, ob Dateiname, Inhalt und öffentliche Bereitstellung zur erwarteten IndexNow-Keydatei passen. |
| **2.8.0 RC** | 🔴 fix | SEO/Security | **Die neue IndexNow-Dateiprüfung begrenzt die Root-Datei jetzt zusätzlich über Lesbarkeit und eine kleine Größenobergrenze**: Unlesbare oder ungewöhnlich große `.txt`-Dateien werden nicht blind als valide Keydatei akzeptiert, bevor ihre Inhalte mit dem API-Key verglichen werden. |
| **2.8.0 RC** | 🔴 fix | SEO/Security | **`CMS/core/Services/IndexingService.php` liest ausgewählte Root-Keydateien jetzt nur noch nach bestandenem Guard vollständig ein**: Wenn Lesbarkeit oder Dateigröße bereits scheitern oder die Dateigröße nicht verlässlich ermittelt werden kann, bleibt der Dateilesepfad geschlossen und meldet stattdessen einen klaren Validierungsfehler. |
| **2.8.0 RC** | 🔴 fix | Review | **Die SEO-/IndexNow-Erweiterung wurde erneut auf Fehler, Best Practice und Security geprüft**: keine Editorfehler, keine PHP-Lint-Fehler; Root-Dateien werden nur basename-basiert aus `ABSPATH` gelesen, Dateiauswahl bleibt allowlist-basiert auf vorhandene `.txt`-Dateien begrenzt und die dynamische `/<key>.txt`-Auslieferung des Cores bleibt unverändert der maßgebliche öffentliche Pfad. |
| **2.8.0 RC** | 🔵 docs | README | **`README.md` wurde deutlich ausgebaut und zweisprachig auf Deutsch und Englisch strukturiert**: Feature- und Technik-Badges, kompakte Audit-Bewertung ohne Tiefendetails, eine öffentliche 5×5-Screenshot-Galerie sowie klarere Architektur-, Dokumentations- und Schnellstartabschnitte machen den Projekteinstieg sichtbarer. |
| **2.8.0 RC** | 🔵 docs | README | **`README.md` beschreibt jetzt zusätzlich den neuen SEO-Workflow für IndexNow-Key- und Root-`.txt`-Prüfung**: Der Projektüberblick nennt die neue Funktion sowohl im deutschen als auch im englischen Bereich. |
| **2.8.0 RC** | 🔵 docs | README | **Die Repository-Struktur im README wurde am Test-Pfad auf `tests/` korrigiert und der SEO-Hinweis um die Guard-Logik ergänzt**: Die Doku beschreibt damit den aktuellen Workspace präziser und nennt den sicheren Prüfpfad ohne verfrühte Dateizugriffe. |
| **2.8.0 RC** | 🔵 docs | Assets | **`DOC/ASSETS_OwnAssets.md` fokussiert den Eigenersatz jetzt auf den aktiven Restbestand unter `CMS/assets/`**: `gridjs`, `photoswipe` und `melbahja-seo` sind als konkrete Kandidaten mit Prioritäten, Zielmodulen und Migrationslogik dokumentiert, während sicherheitskritische Bibliotheken ausdrücklich als Kapselungs- statt Eigenbau-Kandidaten markiert bleiben. |
