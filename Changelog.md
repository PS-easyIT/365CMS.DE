...
| **2.9.248** | 🟢 feat | Admin/Site Tables CMS-Quellen | **`CMS/core/Services/SiteTable/SiteTableContentSource.php`, `CMS/admin/views/tables/edit.php`, `CMS/assets/js/admin-site-tables.js` und `CMS/core/Services/SiteTable/SiteTableTableRenderer.php` erweitern Tabellen um die Option „Als Site Table aus Seiten/Beiträgen verwenden“**: Ist sie aktiv, können Spalten nur noch aus vordefinierten Seiten-/Beitragsfeldern gewählt werden, freie Spalten- und Zelleneingaben sind deaktiviert, und Public sowie Exporte erzeugen die Zeilen dynamisch aus veröffentlichten Seiten und Beiträgen — inklusive `/en/...`-bewusster Ausgabe für echte englische Varianten. |
