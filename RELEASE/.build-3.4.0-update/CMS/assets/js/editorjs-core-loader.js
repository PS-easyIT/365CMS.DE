...
(function () {
    ...
    function recordCoreMilestone(label, details) {
        var debug = ensureEditorDebugChannel();
        var at = new Date().toISOString();

        debug.timestamps[label] = at;
        if (details && typeof details === 'object') {
            debug.timestamps[label + 'Details'] = details;
        }

        if (typeof console !== 'undefined' && typeof console.info === 'function') {
            console.info('[EditorJS][Debug] ' + label + ' @ ' + at, details || null);
        }
    }
    ...
    function logCoreError(message, error) {
        ...
        if (error) {
            console.error('[EditorJS] ' + message, error);
            return;
        }

        console.error('[EditorJS] ' + message);
    }
    ...
    function canUseEvalFallback() {
        return window.cmsEditorJsAllowEvalFallback === true;
    }
    ...
    function loadEditorCoreViaModule() {
        ...
        if (hasEditorCore()) {
            recordCoreMilestone('chainCoreAlreadyPresent', { tag: 'EJS-CHAIN-CORE-READY', coreUrl: coreUrl });
            return Promise.resolve(true);
        }

        recordCoreMilestone('chainModuleImportStart', { tag: 'EJS-CHAIN-MODULE-START', coreUrl: coreUrl });
        return import(coreUrl).then(function (module) {
            applyEditorModule(module);
            recordCoreMilestone('chainModuleImportSuccess', { tag: 'EJS-CHAIN-MODULE-OK', coreUrl: coreUrl });
            return true;
        }).catch(function (directImportError) {
            recordCoreMilestone('chainModuleImportFailed', {
                tag: 'EJS-CHAIN-MODULE-FAIL',
                coreUrl: coreUrl,
                message: directImportError && directImportError.message ? String(directImportError.message) : 'import failed'
            });
    ...
    if (!window.cmsEditorJsCoreReady || typeof window.cmsEditorJsCoreReady.then !== 'function') {
        recordCoreMilestone('coreLoaderPromiseCreated', { hasCore: hasEditorCore() });
        window.cmsEditorJsCoreReady = loadEditorCoreViaModule().catch(function (moduleError) {
            if (hasEditorCore()) {
                recordCoreMilestone('coreReadyRecoveredAfterModuleError');
                return true;
            }

            if (canUseEvalFallback() && loadEditorCoreSynchronously()) {
                recordCoreMilestone('coreReadyViaEvalFallback');
                return true;
            }

            recordCoreMilestone('coreReadyFailed', { reason: 'module-load-failed' });
            logCoreError('EditorJS core could not be loaded.', moduleError);
            throw moduleError;
        }).then(function (ready) {
            if (ready && hasEditorCore()) {
                recordCoreMilestone('coreReady');
            }
