    ...
    deleteCategoryForms.forEach(function(form) {
        form.addEventListener('submit', function(event) {
            ...
            if (!modal) {
                ...
                if (!window.confirm(
                    assignedPosts > 0
                        ? 'Kategorie wirklich löschen? Zugeordnete Beiträge werden in die hinterlegte Ersatzkategorie verschoben.'
                        : 'Kategorie wirklich löschen? Unterkategorien werden zu Hauptkategorien.'
                )) {
                    event.preventDefault();
                }
                return;
