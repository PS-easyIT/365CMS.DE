            ...
            <div class="col-lg-4">
                <div class="card h-100">
                    ...
                    <div class="card-body d-flex flex-column gap-3">
                        <div class="performance-cache-legend">Blau = alle Layer · Outline = einzelner Layer · Orange = OPcache · Teal = Warmup</div>
                        <form method="post" data-confirm-title="Alle Cache-Layer leeren" data-confirm-message="<?php echo htmlspecialchars($buildConfirmMessage('Wirklich Datei-Cache plus flüchtige Runtime-Caches leeren? Vorab wird automatisch ein Datei-Cache-Snapshot für ein zeitlich begrenztes Rollback angelegt.'), ENT_QUOTES); ?>" data-confirm-text="Jetzt leeren" data-confirm-class="btn-primary" data-confirm-status-class="bg-primary"><input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($csrfToken ?? ''); ?>"><input type="hidden" name="action" value="clear_all_cache"><button type="submit" class="btn btn-primary w-100">Alle Cache-Layer leeren</button></form>
                        ...
                        <form method="post" data-confirm-title="OPcache zurücksetzen" data-confirm-message="<?php echo htmlspecialchars($buildConfirmMessage('OPcache wirklich zurücksetzen? Kompilierte PHP-Skripte werden neu aufgebaut und der Warmup startet anschließend erneut.'), ENT_QUOTES); ?>" data-confirm-text="OPcache leeren" data-confirm-class="btn-warning" data-confirm-status-class="bg-warning"><input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($csrfToken ?? ''); ?>"><input type="hidden" name="action" value="clear_opcache"><button type="submit" class="btn btn-warning w-100">OPcache zurücksetzen</button></form>
                        <form method="post"><input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($csrfToken ?? ''); ?>"><input type="hidden" name="action" value="warmup_opcache"><button type="submit" class="btn btn-teal w-100">Top-30 PHP-Dateien vorwärmen</button></form>
                        <?php if ($hasRollback): ?>
