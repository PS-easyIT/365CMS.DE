...
final class AiSettingsService
{
    ...
    private function defaultTranslation(): array
    {
        return [
            ...
            'supported_block_types' => ['paragraph', 'header', 'list', 'checklist', 'quote', 'image', 'table', 'attaches', 'linkTool', 'callout', 'warning', 'alert', 'accordion', 'imageGallery', 'mediaText'],
            'preview_required' => true,
    ...
    private function defaultPromptTemplates(): array
    {
        return [
            ...
            'content_creator' => [
                'enabled' => true,
                'label' => 'Content-Entwürfe',
                'system_prompt' => 'You are a CMS content assistant. Create factual, editorial draft suggestions for human review using only the supplied briefing and context. Do not invent claims, data, people, quotes, URLs, prices or guarantees. Never publish or save content directly. Return JSON only.',
                'user_template' => "TASK: {format}\nTONE: {tone}\n\nCONTENT_BRIEF:\n{content_brief}\n\nCONTEXT:\n{context}",
                'notes' => 'Produktive Vorlage für ungespeicherte Kurzfassungen, Gliederungen und CTA-Varianten im geschützten AI-Adminbereich. Veröffentlichung oder automatische Übernahme ist ausgeschlossen.',
                'placeholders' => ['{content_brief}', '{context}', '{tone}', '{format}'],
            ],
            'seo_creator' => [
                'enabled' => true,
                'label' => 'SEO-Metadaten aus Haupttext',
                'system_prompt' => 'You are a CMS SEO metadata assistant. Analyze only the primary content supplied in the user message. Create factual, concise metadata for human review. Never invent facts, URLs, image URLs, people, certifications, statistics or quotes. Return JSON only.',
                'user_template' => "TASK: Generate reviewable SEO metadata from this CMS primary content.\nCONTENT_TYPE: {content_type}\nLOCALE: {locale}\n\nPRIMARY_CONTENT:\n{context}",
                'notes' => 'Produktive Vorlage für SEO-Metadaten aus Editor.js-Haupttext. Die Runtime erzwingt eine Feld-Whitelist; Dokumenttitel, Slug sowie URL- und Bildfelder bleiben unverändert. Dauerhaft gespeichert wird erst mit dem normalen Formular-Submit.',
