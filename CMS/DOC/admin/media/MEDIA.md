...
Der gemeinsame Featured-Image-Picker für Seiten und Beiträge akzeptiert nur die Backend-Bildformate JPG, PNG, GIF, WebP, BMP und ICO. Bei neuen Inhalten werden Uploads temporär abgelegt und beim Speichern in den Slug-Ordner verschoben; Verschiebe- oder Metadatenfehler werden protokolliert, sollen aber keine leeren HTTP-500-Antworten nach erfolgreicher Bildübernahme mehr verursachen. Seit `3.0.4` wird vor dem temporären Speichern zusätzlich geprüft, ob derselbe Bildinhalt bereits als permanentes Beitrags- oder Seitenbild existiert. In diesem Fall wird die vorhandene Medienreferenz zurückgegeben, statt denselben Inhalt ein weiteres Mal abzulegen. Seit `3.0.24` liefert der Picker öffentliche Bildreferenzen als direkte relative `/uploads/...`-URLs aus und zieht beim finalen Verschieben die Dateirechte erneut nach, damit Aktualisieren-POSTs keine unnötigen Delivery-Query-URLs transportieren und öffentliche Bilder webserverlesbar bleiben.
