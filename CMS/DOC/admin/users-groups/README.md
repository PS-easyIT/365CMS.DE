...
- Die Oberflächen `/admin/users`, `/admin/groups`, `/admin/roles` und `/admin/user-settings` folgen jetzt konsistent dem klassischen Admin-Flow `Header → Toolbar/Filter → Inhalt`; dekorative KPI-Kachelreihen wurden auf den Hauptseiten zugunsten kompakter Header-Metazeilen und klarer Steuerleisten reduziert.
